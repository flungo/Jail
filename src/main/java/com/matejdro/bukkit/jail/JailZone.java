...
public class JailZone {
     ...
     * @param pteleX X coordinate of the location, where players get teleported
     * after being jailed in this jail zone
     * @param pteleY Y coordinate of the location, where players get teleported
     * after being jailed in this jail zone
     * @param pteleZ Z coordinate of the location, where players get teleported
     * after being jailed in this jail zone
     * @param pfreeX X coordinate of the location, where players get teleported
     * after being released from this jail zone
     * @param pfreeY Y coordinate of the location, where players get teleported
     * after being released from this jail zone
     * @param pfreeZ Z coordinate of the location, where players get teleported
     * after being released from this jail zone
     * @param pteleWorld name of the world, which contains the location, where
     * players get teleported after being jailed in this jail zone
     * @param pfreeWorld name of the world, which contains the location, where
     * players get teleported after being released from this jail zone
     */
    ...
    /**
     * @return Location, where players get teleported after being jailed in this
     * jail zone. Overriden if player is jailed into cell.
     */
    ...
    /**
     * Change location, where players get teleported after being jailed in this
     * jail zone
     *
    ...
    /**
     * @return location, where players get teleported after being released in
     * this jail zone
     */
    ...
    /**
     * Change location, where players get teleported after being released in
     * this jail zone
     *
     * @param input New teleport location
     */
    ...
    public JailCell getEmptyCell() {
        for (JailCell c : cells) {
            if (c.getName() != null && getSettings().getList(Setting.ManualCells).contains(c.getName())) {
                continue;
            }
    ...
    /**
     * @param prisoner
     * @return cell that jailer chosen for this prisoner. Will be null if there
     * is no such cell.
     */
    public JailCell getRequestedCell(JailPrisoner prisoner) {
        for (JailCell cell : getCellList()) {
            if (cell.getName() != null && cell.getName().equals(prisoner.getRequestedCell())) {
                return cell;
            }
            if (cell.getPlayerName() != null && cell.getPlayerName().equals(prisoner.getName())) {
                return cell;
            }
        }
        return null;
    ...
    /**
     * @return nearest cell to the specified location. Measured using teleport
     * location.
     */
    ...
    /**
     * @param loc Location to check
     * @return squared distance between teleport location of this jail zone and
     * specified location in blocks. If locations are not in same world,
     * distance cannot be calculated and it will return 2147483647.
     */
    public double getDistance(Location loc) {
        if (loc.getWorld() != getTeleportLocation().getWorld()) {
    ...
    /**
     * Update properties of this jail zone into database. Remember to do that
     * after doing your changes, or changes will revert after server restart.
     */
    public void update() {
