...
public class JailZoneManager {
    ...
    public static JailZone findNearestJail(Location loc) {
        ...
        for (JailZone i : Jail.zones.values()) {
            if (((List<String>) Settings.getGlobalList(Setting.ManualJails)).contains(i.getName())) {
                continue;
            }
    ...
    public static JailZone findNearestJail(Location loc, String ignore) {
        ...
        for (JailZone i : Jail.zones.values()) {
            if (i.getName().equalsIgnoreCase(ignore)) {
                continue;
            }
            if (((List<String>) Settings.getGlobalList(Setting.ManualJails)).contains(i.getName())) {
                continue;
    ...
    /**
     * Return jail that contains location. Returns null if there is no such
     * jail.
     */
