...
public class PrisonerManager {
    ...
    /**
     * Performs jailing of specified JailPrisoner. If you just want to jail
     * someone, I recommend using JailAPI.jailPlayer, because it supports
     * offline jail and it's easier to do.
     *
     * @param prisoner JailPrisoner class of the new prisoner. Must be already
     * inserted into database
     * @param player Player that will be teleported
     */
    @SuppressWarnings("unchecked")
    public static void Jail(JailPrisoner prisoner, Player player) {
        if (!prisoner.getName().equals(player.getName().toLowerCase())) {
            return;
        }
        ...
        if (cell != null) {
            ...
            if (jail.getSettings().getBoolean(Setting.StoreInventory) && cell.getChest() != null) {
                ...
                for (int i = 0; i < 40; i++) {
                    if (chest.getInventory().getSize() <= Util.getNumberOfOccupiedItemSlots(chest.getInventory().getContents())) {
                        break;
                    }
    ...
    /**
     * Performs releasing of specified JailPrisoner. If you just want to release
     * someone, I recommend using prisoner.release, because it supports offline
     * release and it's easier to do.
     *
    ...
    /**
     * Initiate transfer of every prisoner in specified jail zone to another
     * nearest jail zone
     */
    ...
    /**
     * Performs transfer of specified JailPrisoner. If you just want to transfer
     * someone, I recommend using prisoner.transfer, because it supports offline
     * transfer and it's easier to do.
     *
     * @param prisoner Prisoner that will be transfered
     * @param player Player that will be teleported
     */
    public static void Transfer(JailPrisoner prisoner, Player player) {
        if ("find nearest".equals(prisoner.getTransferDestination())) {
            prisoner.setTransferDestination(JailZoneManager.findNearestJail(player.getLocation(), prisoner.getJail().getName()).getName());
        }
